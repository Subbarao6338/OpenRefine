...
class RefineServerTest(refinetest.RefineTestCase):
    def test_init(self):
        ...
        # strip trailing /
        server = refine.RefineServer('http://127.0.0.1/')
        self.assertEqual(server.server, 'http://127.0.0.1/')
    ...
    def test_version(self):
        self.assertTrue(self.server.version in ('2.7'))
